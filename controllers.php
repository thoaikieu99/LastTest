...
class Database
{
    ...
    public function queryCosti($sql = '')
    {
...
function updateIma()
{
    ...
    $ss = "[]";
    $db = new Database();
    $sql = "SELECT `image`, slug, id FROM `Audios` WHERE `image` LIKE 'nonImage.jpg' ORDER BY `id` ASC";
    ...
    foreach ($isIsset as $resource) {
        ...
        $result  = json_decode($content);
        $date = $result[0]->featured_media;


        $simage = image($resource["slug"],  $date);
        $dataa = [
            'image'  => $simage
        ];

        $upa = new Database();
        if ($upa->update('Audios', $dataa, $resource["id"])) {
            echo ($resource["slug"]);
            continue;
        } else {
            return "Audios2dd errors ";
        }
    }
...
function getArch()
{
    ...
    $result  = json_decode($content);
    if (!isset($result->files)) {
        return "errors archive.org/metadata:" . $linkA;
    } else {
        ...
        if ($isIsset) {
            if ($isIsset && $idUpadate) {
                ...
                if (!isset($result->files)) {
                    ...
                    return "errors non-metaLink: " . $isIsset["metalink"];
                }
                $resultt = $result->files;
                $link = array();
                foreach ($resultt as $resource) {
